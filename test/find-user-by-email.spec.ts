...
describe('Find Salesforce contacts by email', () => {
    ...
    it('Finding a SF contact by email for a contact that does not exist returns 404 Not Found', async () => {
        ...
        const response: request.Response = await requester.get('/salesforce/contact/test@email.com');
    ...
    it('Finding a SF contact by email for a contact that exists returns 200 OK with the contact information', async () => {
        const sfContact: SFContact = SFContactFactory.get();

        // Ensure stub is called before starting the test server
        stubJSForce(sandbox, [sfContact]);

        requester = await getTestAgent(true);

        const response: request.Response = await requester.get('/salesforce/contact/test2@email.com');
